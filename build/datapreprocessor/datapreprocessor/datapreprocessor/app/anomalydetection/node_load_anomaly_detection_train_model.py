...
from oedianl.app.nodeload.timeseries_data_utilities import get_config_dict,get_time_series_dataframe
...
from oedianl.app.nodeload.nodeload_utilities import create_average_timeseries_profiles,check_and_create_folder,generate_load_node_profiles
from oedianl.app.dataimputation.data_imputation_preprocessing import get_df_node_load_selected_nodes
...
folder_name_plots = os.path.join(workDir,"app","anomalydetection","plots")
...
n_days = config_dict["train_data_details"]["n_days"] #4 #The number of individual node load profiles that will be generated for training
...
folder_name_saved_models = os.path.join(workDir,"app","anomalydetection","saved_models",distribution_system,f'month_{selected_month}')
...
df_averaged_load,df_averaged_day_load = create_average_timeseries_profiles(timeseries_files=selected_timeseries_files,convert_to_kW=True,upsample=upsample_original_time_series,upsample_time_period=upsample_time_period)
...
input_features = ["load_value"] + auxiliary_features  + encoded_cyclical_features
...
selected_train_nodes,selected_test_nodes,selected_eval_nodes = get_train_test_eval_nodes(load_node_dict,train_fraction=0.8,test_fraction=0.2)

## Generate training and testing data
df_train = get_df_node_load_selected_nodes(df_node_load,cyclical_features,selected_train_nodes,corrupted_fraction=0.0) #We use same method as data imputation but with nor data corruption
df_test = get_df_node_load_selected_nodes(df_node_load,cyclical_features,selected_test_nodes,corrupted_fraction=0.0) #We use same method as data imputation but with nor data corruption
...
#autoencoder.compile(optimizer='adam', loss=tf.keras.losses.MeanSquaredError(),metrics=[tf.keras.metrics.MeanAbsoluteError()]) #losses.MeanAbsoluteError()

monitored_metric = "val_loss"#"val_mean_absolute_error" #"val_loss"
checkpoint_file = 'ad_multi_model.epoch{epoch:02d}-loss{val_loss:.5f}' #'-mae{val_mean_absolute_error:.5f}'
checkpoint = tf.keras.callbacks.ModelCheckpoint(filepath=os.path.join(folder_name_saved_models,f'm-{calendar.month_abbr[selected_month]}_w-{load_block_length}_f-{n_input_features}_n-{n_train_samples}_{model_type}_'+checkpoint_file),
...
history = autoencoder.fit(train_input_target,epochs=n_epochs,shuffle=True,validation_data=test_input_target,callbacks=callbacks)
toc = time.perf_counter()

plot_training_history(history,os.path.join(folder_name_plots,f"loss_history_{distribution_system}_m-{calendar.month_abbr[selected_month]}_w-{load_block_length}_f-{n_input_features}_n-{n_train_samples}_{model_type}.png"))
...
prediction_model_folder = os.path.join(folder_name_saved_models,f'm-{calendar.month_abbr[selected_month]}_w-{load_block_length}_f-{n_input_features}_n-{n_train_samples}_{model_type}_ad_multi_model.epoch{best_epoch:02d}-loss{best_val_loss:.5f}') 

model_archive = os.path.join(workDir,"app","anomalydetection","model",f'ad_model-{model_type}_dss-{distribution_system}_m-{calendar.month_abbr[selected_month]}_w-{load_block_length}_f-{n_input_features}_{model_identifier}.7z')
