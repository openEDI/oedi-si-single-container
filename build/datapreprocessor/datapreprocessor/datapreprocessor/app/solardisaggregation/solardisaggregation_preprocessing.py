...
from datapreprocessor.datapreprocessor.app.nodeload.nodeload_utilities import plot_distribution,get_upsampled_df,get_rated_node_loads,find_unique_loads
...
def convert_solardata_to_timeseries(df,n_days,n_customers = 10,show_details=False,rename_dict = {}):
	...
	df_timeseries.insert(0, "datetime", df_timeseries['date_block'] + "-" + df_timeseries['time_block']) #Insert date time block
	df_timeseries['datetime']  = pd.to_datetime(df_timeseries['datetime'], format='%d/%m/%Y-%H:%M:%S')
...
def generate_solar_array(df_timeseries,available_solar_types,min_n_solar_types=2,max_n_solar_types=5,show_details=False,solar_column_identifier ="solar_power_customer",load_column_identifier ="gross_load_customer"):
	...
	df_solar["datetime"] = df_timeseries["datetime"]
...
def generate_solar_node_profiles(df_timeseries,casefile,selected_months = [1],n_solar_nodes=10,max_solar_penetration = 0.4,upsample_time_series=False,upsample_time_period="15Min"):
	...
	P,Q = get_rated_node_loads(case_file=casefile,n_nodes=n_solar_nodes)

	# -ve number implies n_solar_nodes=number of nodes
	if n_solar_nodes<0: # add all nodes
		n_solar_nodes=len(P)
	...
	if upsample_time_series:
		df_timeseries = get_upsampled_df(df_timeseries,datetime_column="datetime",upsample_time_period="15Min")
	...
	assert len(available_solar_capacities) > 2, "Expected more than 2 capacities"
	...
	for node in tqdm(solar_node_dict.keys()):
		...
		selectedCols=colChoice[0:np.random.randint(2,min(20,len(available_solar_capacities)))]
		total_solar_power = pd.to_numeric(df_timeseries.loc[:, selectedCols].sum(axis=1),downcast='float') #float32, 4-bytes
		total_gross_load = pd.to_numeric(df_timeseries.loc[:,\
			[entry.replace('solar_power','gross_load') for entry in selectedCols]].sum(axis=1),downcast='float') #float32, 4-bytes
		solar_scaling_factor = (P[node]*solar_node_dict[node]["solar_penetration"])/total_solar_power.values.max() #Calcuat scaling factor for solar power
