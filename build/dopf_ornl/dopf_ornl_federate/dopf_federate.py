...
logConfig={
	"logLevel": 20, 
	"logFilePath": os.path.join(baseDir,f'ornl_dopf_federate.log'), 
	"mode": "w", 
	"loggerName": "ornl_dopf_federate_logger"
}
	...
	def __init__(self,config,dt=1):
		try:
			...
			self.requiredOutput={'dsse':['VoltagesMagnitude','VoltagesAngle'],\
				'dopf':['PowersReal','PowersImaginary']}
		...
		try:
			...
			if 'dynamic_inputs' in self.config['component_definition']:
				for thisInput in self.config['component_definition']['dynamic_inputs']:
					LogUtil.logger.warn(f"config::::{self.config}")
					self.config['inputPort2TypeMapping'][self.config['input_mapping_ext'][thisInput['port_id']]]=thisInput['type']

			LogUtil.logger.warn(f"completed inputPort2TypeMapping::::{self.config['inputPort2TypeMapping']}")
			...
			LogUtil.logger.warn(f"completed outputPort2TypeMapping::::{self.config['outputPort2TypeMapping']}")
		...
		try:
			...
			while grantedTime<=simEndTime:
				...
				if subData:
					...
					outputData=algorithmObj(inputData,LogUtil.logger)
